...
codonstats <- select(corp, sero, codon, p) %>% 
  ...
  left_join(read.csv(file='construct_v_WT_change', head=TRUE, sep='\t'))
```

Here are plots comparint slope, rtAI, and CAI.  I'm expecting positive slopes for rtAI vs slope, negative for the rest I guess.
...
Here are plots comparing the change between my construct and the WT with the rtAI, CAI and slope.

```{r}
ggplot(compareconstruct, aes(x=change, y=rtAI, label=codon))+geom_point()+ background_grid(major = "xy", minor = "none")+geom_text(size = 5)+facet_wrap(~sero)
ggplot(compareconstruct, aes(x=change, y=CAI, label=codon))+geom_point()+ background_grid(major = "xy", minor = "none")+geom_text(size = 5)+facet_wrap(~sero)
ggplot(compareconstruct, aes(x=change, y=slope, label=codon))+geom_point()+ background_grid(major = "xy", minor = "none")+geom_text(size = 5)+facet_wrap(~sero)
```

I'm really not sure hot to interpret this, though...