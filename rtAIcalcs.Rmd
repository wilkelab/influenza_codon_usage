...
rtPB1 %>% group_by(sero, strain) %>% 
  ...
  ggplot(aes(x=coryear, y=score, color=sero)) + geom_point() +geom_smooth(method='lm', se=FALSE)
...
rtPB1 %>% group_by(sero, strain) %>% 
  filter(codon!='ATG', codon!='TGG', codon!='TAA', codon!='TAG', codon!='TGA') %>%
  summarize(coryear=mean(coryear),
            score=(sum(numcodon*Altw.IFNratiolib2/Altw.AnoIFNratiolib2))/sum(numcodon)) %>%
  ggplot(aes(x=coryear, y=score, color=sero)) + geom_point() +geom_smooth(method='lm', se=FALSE)
...
rtPB1 %>% 
  ...
  summarize(coryear=mean(coryear),
            score=(sum(numcodon*Altw.IFNratiolib2/Altw.AnoIFNratiolib2))/sum(numcodon)) %>%
  ggplot(aes(x=coryear, y=score, color=sero)) + geom_point() + ylim(.965, .98) +geom_smooth(method='lm', se=FALSE)

rtPB1 %>% 
  filter(codon!='ATG', codon!='TGG', codon!='TAA', codon!='TAG', codon!='TGA') %>% 
  filter(Altw.AIFNreadslib2 >= 1000, Altw.AnoIFNreadslib2 >= 1000) %>% 
  group_by(sero, strain) %>%
  summarize(coryear=mean(coryear),
            score=(sum(numcodon*Altw.IFNratiolib2/Altw.AnoIFNratiolib2))/sum(numcodon)) %>%
  ggplot(aes(x=coryear, y=score, color=sero)) + geom_point() +geom_smooth(method='lm', se=FALSE)
...
Alright, lets get some slopes n stuff.  I'm sure this is an insanely inefficient way of doing this...  All I gotta do though is cutnpaste the summaries for the plots down below, right before -> scores
```{r}
rtPB1 %>% 
  filter(codon!='ATG', codon!='TGG', codon!='TAA', codon!='TAG', codon!='TGA') %>% 
