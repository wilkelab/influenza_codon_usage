...
rtPB1 %>% 
  ...
  ggplot(aes(x=coryear, y=score, color=sero)) + 
  geom_point() + 
...
rtPB1 %>% group_by(sero, strain) %>% 
  filter(codon!='ATG', codon!='TGG', codon!='TAA', codon!='TAG', codon!='TGA') %>%
  summarize(coryear=mean(coryear),
            score=(sum(numcodon*AIFNratiolib2/AnoIFNratiolib2))/sum(numcodon))  -> scores
  
scores %>% ggplot(aes(x=coryear, y=score, color=sero)) + geom_point() +geom_smooth(method='lm', se=FALSE)
scores %>% group_by(sero) %>% do(lm.result = lm((score) ~ coryear, data=.)) -> fitted.models
intslope <- do(fitted.models, data.frame(sero=.$sero, 
                             intercept=coef(.$lm.result)[1], 
                             slope=coef(.$lm.result)[2]))
rm(fitted.models)
scores %>% do(cor.res = cor.test(.$score, .$coryear)) -> cors
corp <- do(cors, data.frame(sero=.$sero, cor=.$cor.res$estimate, p=.$cor.res$p.value))
rm(cors)
ynp <- select(intslope, sero, slope) %>% left_join(select(corp, sero, p))
rm(corp)
rm(intslope)
rm(scores)
ynp
...
*rtAI graphs based on newer library, different wobble calling*
Cutoffs for read # are 0, 500 (removes one codon), 1000 (removes 8 codons).
```{r}
rtPB1 %>% group_by(sero, strain) %>% 
  filter(codon!='ATG', codon!='TGG', codon!='TAA', codon!='TAG', codon!='TGA') %>%
  summarize(coryear=mean(coryear),
            score=(sum(numcodon*Altw.IFNratiolib2/Altw.AnoIFNratiolib2))/sum(numcodon)) -> scores
...
rtPB1 %>% 
  filter(codon!='ATG', codon!='TGG', codon!='TAA', codon!='TAG', codon!='TGA') %>% 
  filter(Altw.AIFNreadslib2 >= 500, Altw.AnoIFNreadslib2 >= 500) %>% 
  group_by(sero, strain) %>%
  summarize(coryear=mean(coryear),
            score=(sum(numcodon*Altw.IFNratiolib2/Altw.AnoIFNratiolib2))/sum(numcodon)) -> scores
  
scores %>% ggplot(aes(x=coryear, y=score, color=sero)) + geom_point() +geom_smooth(method='lm', se=FALSE) +ylim(.965, .98)
scores %>% group_by(sero) %>% do(lm.result = lm((score) ~ coryear, data=.)) -> fitted.models
intslope <- do(fitted.models, data.frame(sero=.$sero, 
                             intercept=coef(.$lm.result)[1], 
                             slope=coef(.$lm.result)[2]))
...
rtPB1 %>% 
  filter(codon!='ATG', codon!='TGG', codon!='TAA', codon!='TAG', codon!='TGA') %>% 
  filter(Altw.AIFNreadslib2 >= 10000, Altw.AnoIFNreadslib2 >= 10000) %>% 
  group_by(sero, strain) %>%
  summarize(coryear=mean(coryear),
            score=(sum(numcodon*Altw.IFNratiolib2/Altw.AnoIFNratiolib2))/sum(numcodon)) -> scores
scores %>% ggplot(aes(x=coryear, y=score, color=sero)) + geom_point() +geom_smooth(method='lm', se=FALSE)
...
Alright, lets get some slopes n stuff.  I'm sure this is an insanely inefficient way of doing this...  All I gotta do though is cutnpaste the summaries for the plots down below, right before -> scores.   Actually, I'm gonna dump all this back up into those graphs directly...
```{r}
rtPB1 %>% 
  filter(codon!='ATG', codon!='TGG', codon!='TAA', codon!='TAG', codon!='TGA') %>% 
...
When I filter by 10,000 reads, the whole thing looks pretty good, but that's removing 8 codons from the analysis.  Those're low frequency tRNAs and it might be that my library prep just didn't get a representative sample from them.